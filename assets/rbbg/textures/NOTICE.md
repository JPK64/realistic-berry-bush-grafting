Asset Copyright Notice:

Certain textures, models, or visual elements contained within this
directory are modifications of assets originally created by Anego
Studios for Vintage Story.

- All rights to the original and modified base game assets remain the
  property of Anego Studios.
- These assets are permitted for use strictly within the Vintage Story
  modding ecosystem in accordance with Anego Studios' policies. 
- They are explicitly excluded from the CC BY-SA license governing the
  rest of the mod's content and cannot be extracted for use in external
  games or software.
